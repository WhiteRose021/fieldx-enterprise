...
const SidebarItem = ({ item, pageName, setPageName }: SidebarItemProps) => {
  ...
  const isFTTHItem = item.label.includes('FTTH') || item.label.includes('Phase');
  const itemClasses = `${
  ...
  return (
    <li>
      ...
      >
        {item.icon}
        <span className={isFTTHItem ? 'font-bold' : ''}>{item.label}</span>
        {item.children && (
          <svg
      ...
      {item.children && (
        ...
        >
          <SidebarDropdown 
            item={item.children}
            parentLabel={item.label} // Pass parent label to identify FTTH items
          />
        </div>
      )}
    </li>
